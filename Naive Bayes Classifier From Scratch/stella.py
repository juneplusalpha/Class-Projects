...
class LogisticRegression(Classifier):
    ...
    def __predict(self, weights):
        ...
        scores = features.dot(weights[1:])
        logging.debug(features)
        predictions = self.__sigmoid(scores)
        logging.debug(predictions)
        logging.debug("Features len: {} Pred len: {}".format(len(self.dev_inst), len(predictions)))
        pred_label =[]
    ...
    def process(self, num_steps = 300, learning_rate = 5e-5):
        ...
        weights = self.__logistic_regression(features, target, learning_rate, num_steps)
